...
- **Open it and present** — six diagram types, including UML-style class diagrams, four presets, dark/light themes, built-in brand marks, and finite motion
...
The [Proof Lab](https://tt-a1i.github.io/archify/gallery.html) contains all 12 checked-in scenarios, their JSON sources, named views, and validation receipts.
...
For a Java/Spring repository, generate endpoint-focused class diagrams grouped by controller. Every endpoint gets an isolated horizontal scenario lane with shared types duplicated and trimmed to relevant members; controllers with more than three routes split automatically. Use `--controller TraineeController` for one controller or `--locale ru` for a Russian index. See the [endpoint extraction contract](archify/references/endpoint-extraction.md).
