...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.deepEqual(
    Object.fromEntries(['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'class-diagram'].map((type) => [
      type,
      manifest.entries.filter((entry) => entry.type === type).length,
    ])),
    { architecture: 2, workflow: 3, sequence: 2, dataflow: 2, lifecycle: 2, 'class-diagram': 1 },
  );
  assert.deepEqual(
  ...
  for (const type of ['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'class-diagram']) {
